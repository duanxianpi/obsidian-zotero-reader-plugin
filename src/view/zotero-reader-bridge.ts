...
import type { ChildApi, ParentApi, CreateReaderOptions, Theme, ChildEvents, InitPayload } from './zotero-reader';
...
export class IframeReaderBridge {
  ...
  async connect(initPayload: InitPayload) {
    ...
    const remote = await Promise.race([
      remotePromise,
      new Promise<never>((_, rej) => setTimeout(() => rej(new Error('Child connect timeout')), this.connectTimeoutMs)),
    ]);
    this.remote = remote;

    // Initialize child
    await this.remote.init(initPayload);
    this.state = 'ready';
  ...
  private enqueueOrRun(fn: () => Promise<void>) {
    ...
    if (this.state === 'connecting') { this.queue.push(fn); return Promise.resolve(); }
    return Promise.reject(new Error(`Bridge not ready (state=${this.state})`));
  }

  createReader(opts: CreateReaderOptions) {
    return this.enqueueOrRun(async () => {
      await this.remote!.createReader(opts);
    });
  }
